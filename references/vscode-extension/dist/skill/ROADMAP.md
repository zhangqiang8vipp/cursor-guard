...
>
> **当前版本**：`V4.9.7`
> **文档状态**：`V2` ~ `V4.9.7` 已完成交付（含 V5 intent/audit 基础），`V5` 主体规划中
...
| 修复/增强 | 说明 |
|----------|------|
| **esbuild 单文件 bundle** | 彻底解决 VSIX MCP 依赖缺失问题。不再手动拷贝 `vendor/` 目录（`@modelcontextprotocol/sdk`、`zod`、`zod-to-json-schema`、`ajv` 等），改用 esbuild 将 `mcp/server.js` 及其所有依赖打成一个自包含的 CJS bundle（~1.5 MB）。任何新增的 SDK 传递依赖都会自动包含 |
| **VSIX 体积大幅缩减** | 从 1543 文件 / 3.18 MB 缩减到 42 文件 / 1.27 MB（-60%）。vendor/ 目录完全移除 |
| **auto-setup.js 简化** | MCP server 注册不再需要 `NODE_PATH` 环境变量（bundle 自包含），命令从 `{ command: "node", args: [...], env: { NODE_PATH: "..." } }` 简化为 `{ command: "node", args: [...] }` |
| **build-vsix.js 容错** | esbuild 失败时自动 fallback 到源码拷贝，不会中断打包流程 |
...
V4.9.7 ─────  ✅ 预警体验打磨 + 语言同步 + watcher 单例保护  ← 当前版本
               │
...
*最后更新：2026-03-22*
*版本：v1.7（V4.9.7，含事件驱动 watcher、自触发备份反馈环修复、`pre_warning` 事先预警 MVP，以及预警 UX/Watcher 单例修复）*
